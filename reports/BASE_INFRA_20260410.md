...
| Field Coherence (κ) | 0.2517 | low coherence — entity has limited connection to measured field activity |
...
κ = 0.2517 reflects the engine's current **measurement coverage**, not Base's actual institutional influence. With no vault records yet, the coherence score is derived purely from topology position — connection count and structural weight — without any attested observation data. The low value means the engine has not yet measured this entity's settlement activity, not that the entity lacks field presence.

The equilibrium attractor is κ = 1/φ ≈ 0.618. At current κ = 0.2517, Base is **0.3663 below** equilibrium. Vault observations of Base's rule outputs — rulemaking notices, enforcement actions, guidance documents — would directly raise κ toward the living zone [0.618, 0.678].
...
Base ranks **#7** in the field's power concentration with a score of 1.75 (field total: 80.43).
...
*Generated: 2026-04-10T21:08:23 UTC by Eigenstate engine · Report ID: BASE_INFRA_20260410*