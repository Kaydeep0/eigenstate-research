...
class BinaryTunnel {
    ...
    drawFloatingEquations() {
        ...
        // Distance thresholds (as % of screen diagonal)
        const minVisibleDist = 150; // Start becoming visible
        const fullFormDist = 350;   // Fully formed equation
        const maxDist = 600;        // Reset distance
        ...
        this.equations.forEach(eq => {
            ...
            eq.dist += eq.speed * this.warpMultiplier;
            
            // Reset when too far
            if (eq.dist > maxDist) {
                eq.dist = 0;
                eq.angle += 0.5; // Slightly different angle each time
                eq.formProgress = 0;
            }
            ...
            let opacity = 0;
            if (eq.dist > minVisibleDist) {
                opacity = Math.min(0.6, (eq.dist - minVisibleDist) / 200);
            }
            
            if (opacity < 0.02) return; // Skip if invisible
            
            // Form progress: how much of the equation is "resolved"
            eq.formProgress = Math.min(1, (eq.dist - minVisibleDist) / (fullFormDist - minVisibleDist));
            
            // Generate display text (binary chaos → equation)
            let displayText = '';
            ...
            // Size based on distance (smaller when far)
            const size = 10 + (eq.dist / maxDist) * 6;
            ...
            // Glow when more formed
            if (eq.formProgress > 0.5) {
                this.ctx.shadowBlur = 12 * eq.formProgress;
