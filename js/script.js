...
class BinaryTunnel {
    constructor(canvasId) {
        ...
        // Canvas context optimization: alpha: false for better performance
        this.ctx = this.canvas.getContext('2d', { alpha: false });
        this.particles = [];
        // Nuclear option: Further reduced particle count for maximum performance
        // Was: 800 → 500/300 → Now: 200/120
        this.particleCount = window.innerWidth < 768 ? 120 : 200;
        
        // Additional optimization options (commented out - enable if needed):
        // Option 2: Skip particle sorting entirely (minor visual degradation)
        // Set to true to disable sorting for better performance
        this.skipSorting = false; // Change to true to disable sorting
        
        // Option 3: Use OffscreenCanvas for worker-thread rendering (advanced)
        // Requires: new OffscreenCanvas() + Worker thread setup
        this.focalLength = 300;
        this.time = 0;
        
        // FPS throttling for performance
        this.lastFrameTime = 0;
        this.targetFPS = 30; // Throttle to 30fps
        ...
        window.addEventListener('click', (e) => {
            ...
            if (e.target.tagName === 'BUTTON' || e.target.tagName === 'A') return;
            // Limit burst count for performance
    ...
    drawFloatingEquations() {
        ...
        this.equations.forEach(eq => {
            ...
            const maxOpacity = isMobile ? 0.3 : 0.75; // Dimmer on mobile
    ...
    draw() {
        ...
        // Sort in-place for better performance (no array copy)
        // Can be disabled for extra performance (minor visual degradation)
        if (!this.skipSorting) {
    ...
    animate(currentTime = 0) {
        ...
        // Skip frame if paused or not visible
...
class NetworkFormation {
    constructor(canvasId) {
        ...
        this.isPaused = false;
        this.lastFrameTime = 0;
        this.targetFPS = 24; // Lower FPS for network animation
        this.frameInterval = 1000 / this.targetFPS;
    ...
    drawPhaseIndicator() {
        ...
        const boxY = 15; // TOP-RIGHT position
