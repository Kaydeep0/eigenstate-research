...
class BinaryTunnel {
    ...
    drawAccretionDisk() {
        ...
        gradient.addColorStop(0.3, 'rgba(255, 107, 53, 0.03)');
        gradient.addColorStop(0.6, 'rgba(255, 80, 30, 0.05)');
        gradient.addColorStop(0.8, 'rgba(255, 60, 20, 0.03)');
        ...
        // Inner bright ring (event horizon edge)
        const horizonGradient = this.ctx.createRadialGradient(cx, cy, innerRadius * 0.95, cx, cy, innerRadius * 1.05);
        horizonGradient.addColorStop(0, 'rgba(255, 150, 80, 0)');
        horizonGradient.addColorStop(0.5, 'rgba(255, 120, 60, 0.08)');
        horizonGradient.addColorStop(1, 'rgba(255, 150, 80, 0)');
    ...
    update() {
        ...
        this.particles.forEach(p => {
            ...
            // Gravitational lensing - bend particles near center
            if (p.z < 800 && p.z > 100) {
                ...
                // Bend toward center when close to the "event horizon"
                const lensRadius = 250;
                if (distFromCenter < lensRadius && distFromCenter > 50) {
                    const bendStrength = (1 - distFromCenter / lensRadius) * 0.15;
                    const angle = Math.atan2(dy, dx);
                    // Spiral bend - perpendicular to radial direction
                    p.x += Math.cos(angle + Math.PI / 2) * bendStrength * (p.z / 100);
                    p.y += Math.sin(angle + Math.PI / 2) * bendStrength * (p.z / 100);
                }
            }
