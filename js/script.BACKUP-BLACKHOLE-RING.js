...
class BinaryTunnel {
    constructor(canvasId) {
        ...
        // Floating equations - emerge from center outward
        this.equations = [
            { text: 'H = -Σp(x)log p(x)', angle: Math.PI * 0.25, dist: 0, speed: 2, delay: 1, formProgress: 0 },
            { text: 'P(A|B) = P(B|A)P(A)/P(B)', angle: Math.PI * 0.75, dist: 0, speed: 2.2, delay: 1.5, formProgress: 0 },
            { text: '∂²ψ/∂t² = c²∇²ψ', angle: Math.PI * 1.25, dist: 0, speed: 1.8, delay: 2, formProgress: 0 },
            { text: 'θ = θ - α∇J(θ)', angle: Math.PI * 1.75, dist: 0, speed: 2.1, delay: 2.5, formProgress: 0 }
        ];
        this.equationTime = 0;
    ...
    drawFloatingEquations() {
        ...
        // Distance thresholds
        const invisibleZone = 120;   // Invisible near center
        const slowdownStart = 180;   // Start slowing here
        const hoverZone = 280;       // Nearly stop here
        const maxDist = 800;         // Eventually reset
        ...
        this.equations.forEach(eq => {
            ...
            let currentSpeed = eq.speed * this.warpMultiplier;
            
            if (eq.dist > slowdownStart) {
                // Dramatic slowdown - the further out, the slower
                const slowFactor = 1 - Math.min(0.95, (eq.dist - slowdownStart) / 150);
                currentSpeed *= slowFactor;
            }
            
            // Minimum creep speed so they eventually reset
            currentSpeed = Math.max(currentSpeed, 0.1);
            
            // Move outward from center
            eq.dist += currentSpeed;
            
            // Reset when very far (takes a long time due to slow speed)
            if (eq.dist > maxDist) {
                eq.dist = 0;
                eq.angle += 0.4; // Different angle each cycle
                eq.formProgress = 0;
            }
            ...
            // Opacity: 0 at center, increases with distance
            let opacity = 0;
            if (eq.dist > invisibleZone) {
                opacity = Math.min(0.55, (eq.dist - invisibleZone) / 120);
            }
            
            if (opacity < 0.02) return;
            
            // Form progress: binary chaos → equation
            eq.formProgress = Math.min(1, (eq.dist - invisibleZone) / 100);
            ...
            // Stable size when hovering
            const size = 11 + Math.min(5, eq.dist / 100);
            ...
            // Soft glow for structure
            if (eq.formProgress > 0.7) {
                this.ctx.shadowBlur = 15;
                this.ctx.shadowColor = `rgba(255, 107, 53, ${opacity * 0.6})`;
            }
