...
class BinaryTunnel {
    ...
    draw() {
        ...
        this.characters.forEach(char => {
            ...
            const spread = char.z * Math.max(this.width, this.height) * 1.1;
...
class DataFlow {
    ...
    draw() {
        ...
        this.particles.forEach(p => {
            // Glow
            const glowSize = p.size * (p.state === 'clustering' ? 2 : 1.2);
            const gradient = this.ctx.createRadialGradient(
                p.x, p.y, 0, p.x, p.y, glowSize
            );
            gradient.addColorStop(0, `rgba(0, 217, 255, ${p.alpha * 0.4})`);
...
class ParticleSystem {
    ...
    animate() {
        ...
        // Draw data flow ecosystem
        if (window.dataFlow) {
            window.dataFlow.draw();
        }
