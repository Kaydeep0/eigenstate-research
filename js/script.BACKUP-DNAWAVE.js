...
class BinaryTunnel {
    ...
    update() {
        ...
        this.particles.forEach(p => {
            ...
            // When approaching focal zone, pull toward center
            if (p.z < this.focalZ + 200 && p.z > this.focalZ - 100) {
                const pullStrength = 1 - Math.abs(p.z - this.focalZ) / 200;
                p.x *= (1 - pullStrength * 0.03);
                p.y *= (1 - pullStrength * 0.03);
            }
            
            // Check if particle reached focal node
            if (p.z < this.focalZ + 50 && p.z > this.focalZ - 50) {
                const dist = Math.sqrt(p.x * p.x + p.y * p.y);
                if (dist < this.focalRadius) {
                    this.nodeGlow += 0.02;
                }
            }
        ...
        this.nodeGlow = Math.min(1, this.nodeGlow);
    }
