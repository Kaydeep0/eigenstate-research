...
class BinaryTunnel {
    constructor(canvasId) {
        ...
        // Vanishing point (center of screen)
        this.vpX = 0;
        this.vpY = 0;
    ...
    handleResize() {
        ...
        // Vanishing point slightly above center for dramatic effect
        this.vpX = this.width / 2;
        this.vpY = this.height * 0.4;
    ...
    createCharacter(forceZ = null) {
        const angle = Math.random() * Math.PI * 2;
        const z = forceZ !== null ? forceZ : Math.random();
    ...
    draw() {
        ...
        this.characters.forEach(char => {
            // Move character toward viewer (increase z)
            char.z += char.speed;
            char.angle += char.rotationSpeed || 0;
            
            // Reset when it passes the viewer
            if (char.z > 1) {
                Object.assign(char, this.createCharacter(0));
            }
            
            // Calculate screen position based on z-depth
            // Characters spread out from center as z increases
            const spread = char.z * Math.max(this.width, this.height) * 0.9;
            ...
            // Size increases with z (perspective)
            const size = char.size * (0.05 + char.z * 0.95);
            
            // Opacity increases with z (closer = brighter)
            const alpha = char.z * char.brightness * 0.9;
            
            // Color gradient: dim cyan at distance, bright white up close
            const whiteness = Math.floor(200 + 55 * char.z);
            const cyan = Math.floor(255 * (1 - char.z * 0.2));
            const color = `rgba(${whiteness}, ${cyan}, 255, ${alpha})`;
            ...
            // Glow effect for close characters
            if (char.z > 0.6) {
                this.ctx.shadowBlur = 20 * char.z;
    ...
    drawDepthLines() {
        // Subtle lines emanating from vanishing point
        const lineCount = 16;
        ...
        for (let i = 0; i < lineCount; i++) {
            ...
            const gradient = this.ctx.createLinearGradient(
                this.vpX, this.vpY,
                this.vpX + Math.cos(angle) * length,
                this.vpY + Math.sin(angle) * length
            );
            gradient.addColorStop(0, 'rgba(0, 200, 255, 0)');
            gradient.addColorStop(0.2, 'rgba(0, 200, 255, 0.015)');
            gradient.addColorStop(1, 'rgba(0, 200, 255, 0)');
            
            this.ctx.strokeStyle = gradient;
            this.ctx.lineWidth = 1;
            this.ctx.beginPath();
            this.ctx.moveTo(this.vpX, this.vpY);
            this.ctx.lineTo(
                this.vpX + Math.cos(angle) * length,
                this.vpY + Math.sin(angle) * length
            );
...
class BinaryWave {
    ...
    createParticles() {
        this.particles = [];
        const spacing = this.isMobile ? 12 : 9;
    ...
    draw() {
        ...
        this.particles.forEach(p => {
            const wave1 = Math.sin(p.baseX * 0.005 + this.time) * 55;
            const wave2 = Math.sin(p.baseX * 0.003 - this.time * 0.7) * 32;
            const wave3 = Math.sin(p.baseX * 0.009 + this.time * 1.4) * 18;
            const wave4 = Math.sin(p.baseX * 0.002 + this.time * 0.4) * 28;
            
            const waveY = this.height * 0.5 + wave1 + wave2 + wave3 + wave4;
            const dist = Math.abs(p.baseY - waveY);
            const prox = Math.max(0, 1 - dist / 85);
            
            if (prox > 0.03) {
                p.y = p.baseY + Math.sin(p.baseX * 0.007 + this.time) * 4;
                p.x = p.baseX + Math.sin(this.time + p.baseY * 0.008) * 2;
                
                if (Math.random() < 0.001) p.char = Math.random() > 0.5 ? '1' : '0';
                
                const bright = 0.12 + prox * 0.88;
                const crest = dist < 8;
                
                if (crest && prox > 0.85) {
                    this.ctx.fillStyle = `rgba(180, 255, 255, ${bright})`;
                    this.ctx.shadowBlur = 10;
                    this.ctx.shadowColor = '#00ffff';
                } else if (prox > 0.55) {
                    this.ctx.fillStyle = `rgba(0, 215, 255, ${bright * 0.9})`;
                    this.ctx.shadowBlur = 3;
                    this.ctx.shadowColor = '#00ffff';
                } else {
                    this.ctx.fillStyle = `rgba(14, ${Math.floor(130 + 90 * prox)}, 233, ${bright * 0.65})`;
                    this.ctx.shadowBlur = 0;
                }
...
class DNAHelix {
    constructor(canvas, x, y, height, rotation = 0) {
        this.canvas = canvas;
    ...
    getNodePosition(node, time) {
        const y = this.centerY - this.height / 2 + node.t * this.height;
        const angle = node.t * Math.PI * 4 + time + (node.strand * Math.PI) + this.baseRotation;
    ...
    draw(globalTime) {
        this.time = globalTime * 0.25;
        
        const sortedNodes = [...this.nodes].map(node => ({
            ...node,
            pos: this.getNodePosition(node, this.time)
        })).sort((a, b) => a.pos.z - b.pos.z);
        
        // Draw base pair connections
        for (let i = 0; i < this.nodeCount; i++) {
            const n0 = sortedNodes.find(n => n.strand === 0 && Math.abs(n.t - i/this.nodeCount) < 0.01);
            const n1 = sortedNodes.find(n => n.strand === 1 && Math.abs(n.t - i/this.nodeCount) < 0.01);
            if (n0 && n1) {
                const isOrange = i % 3 === 0;
                const alpha = 0.2 + (n0.pos.z + 1) * 0.15;
                this.ctx.strokeStyle = isOrange 
                    ? `rgba(255, 107, 53, ${alpha})`
                    : `rgba(0, 217, 255, ${alpha * 0.4})`;
                this.ctx.lineWidth = 1;
                this.ctx.beginPath();
                this.ctx.moveTo(n0.pos.x, n0.pos.y);
                this.ctx.lineTo(n1.pos.x, n1.pos.y);
                this.ctx.stroke();
            }
        }
        ...
        // Draw backbone
        [0, 1].forEach(strand => {
            const strandNodes = sortedNodes.filter(n => n.strand === strand);
            this.ctx.strokeStyle = 'rgba(0, 217, 255, 0.2)';
            this.ctx.beginPath();
            strandNodes.forEach((n, i) => i === 0 ? this.ctx.moveTo(n.pos.x, n.pos.y) : this.ctx.lineTo(n.pos.x, n.pos.y));
            this.ctx.stroke();
        });
        ...
        sortedNodes.forEach(node => {
            ...
            const size = (5 + (node.pos.z + 1) * 2.5) * pulse;
            const alpha = 0.4 + (node.pos.z + 1) * 0.2;
            
            this.ctx.fillStyle = `rgba(0, 217, 255, ${alpha * 0.5})`;
            this.ctx.beginPath();
            this.ctx.arc(node.pos.x, node.pos.y, size * 1.5, 0, Math.PI * 2);
            this.ctx.fill();
            
            this.ctx.fillStyle = `rgba(255, 255, 255, ${alpha})`;
            this.ctx.font = `bold ${Math.max(7, size)}px "JetBrains Mono", monospace`;
            this.ctx.textAlign = 'center';
            this.ctx.textBaseline = 'middle';
            this.ctx.fillText(node.char, node.pos.x, node.pos.y);
        });
    }
}
...
class ParticleSystem {
    ...
    animate() {
        ...
        // Draw DNA helixes
        if (window.dnaHelixes) {
            window.dnaHelixes.forEach(helix => helix.draw(this.time));
        }
    ...
    spawnFlowParticle() {
        if (this.flowParticles.length >= this.maxFlowParticles) return;
    ...
    updateFlowParticles() {
        if (Math.random() < 0.08) this.spawnFlowParticle();
        
        this.flowParticles = this.flowParticles.filter(p => {
            p.wobble += p.wobbleSpeed;
            const wobbleX = Math.sin(p.wobble) * 25;
            ...
            if (dist > 5) {
                p.x += (dx / dist) * p.speed + wobbleX * 0.04;
                p.y += (dy / dist) * p.speed;
            }
            
            if (p.y > this.canvas.height * 0.65) {
                p.alpha = Math.max(0, 1 - (p.y - this.canvas.height * 0.65) / (this.canvas.height * 0.35));
            }
            
            if (p.alpha > 0.05) {
                this.ctx.fillStyle = `rgba(0, 217, 255, ${p.alpha * 0.4})`;
                this.ctx.beginPath();
            ...
            return p.y < this.canvas.height + 50 && p.alpha > 0.01;
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const helixCanvas = document.getElementById('particleCanvas');
    if (helixCanvas) {
        window.dnaHelixes = [];
        const w = window.innerWidth;
        const h = window.innerHeight;
        window.dnaHelixes.push(new DNAHelix(helixCanvas, 100, 200, 380, 0));
        window.dnaHelixes.push(new DNAHelix(helixCanvas, w - 100, 180, 380, Math.PI * 0.5));
        window.dnaHelixes.push(new DNAHelix(helixCanvas, 130, h - 280, 320, Math.PI));
        window.dnaHelixes.push(new DNAHelix(helixCanvas, w - 130, h - 280, 320, Math.PI * 1.5));
    }
