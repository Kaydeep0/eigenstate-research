...
class BinaryTunnel {
    constructor(canvasId) {
        ...
        // Floating equations - fly toward viewer like particles
        this.equations = [
            { text: 'H = -Σp(x)log p(x)', x: 0, y: 0, z: 0, speed: 0, active: false, delay: 2 },
            { text: 'P(A|B) = P(B|A)P(A)/P(B)', x: 0, y: 0, z: 0, speed: 0, active: false, delay: 4 },
            { text: '∂²ψ/∂t² = c²∇²ψ', x: 0, y: 0, z: 0, speed: 0, active: false, delay: 6 },
            { text: 'θ = θ - α∇J(θ)', x: 0, y: 0, z: 0, speed: 0, active: false, delay: 8 }
        ];
        this.equationTime = 0;
    ...
    drawFloatingEquations() {
        ...
        this.equations.forEach(eq => {
            // Activate equation after delay
            if (!eq.active && this.equationTime > eq.delay) {
                eq.active = true;
                // Spawn at random position in the distance
                eq.x = (Math.random() - 0.5) * 800;
                eq.y = (Math.random() - 0.5) * 800;
                eq.z = 1800 + Math.random() * 200;
                eq.speed = 1.5 + Math.random() * 1;
            }
            
            if (!eq.active) return;
            
            // Move toward viewer (same as particles)
            eq.z -= eq.speed * this.warpMultiplier * this.scrollSpeed;
            ...
            // 3D projection (same as particles)
            const scale = this.focalLength / eq.z;
            const screenX = this.centerX + eq.x * scale * this.scrollSpread;
            const screenY = this.centerY + eq.y * scale * this.scrollSpread;
            
            // Size and opacity based on depth (closer = bigger & brighter)
            const size = Math.max(8, Math.min(24, 300 / eq.z * 10));
            const opacity = Math.min(0.6, (1 - eq.z / 2000) * 0.8);
            
            if (opacity < 0.05) return; // Don't draw if too faint
            ...
            if (eq.z < 400) {
                ...
                this.ctx.shadowColor = `rgba(255, 107, 53, ${opacity})`;
            }
            
            this.ctx.fillText(eq.text, screenX, screenY);
            this.ctx.restore();
        });
    }
