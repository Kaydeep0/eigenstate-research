...
class BinaryTunnel {
    constructor(canvasId) {
        console.log('BinaryTunnel: Starting constructor');
        this.canvas = document.getElementById(canvasId);
        console.log('BinaryTunnel: Canvas found:', !!this.canvas);
        if (!this.canvas) return;
        
        console.log('BinaryTunnel: Getting context');
        this.ctx = this.canvas.getContext('2d');
        console.log('BinaryTunnel: Context:', !!this.ctx);
        this.particles = [];
        ...
        // Floating equations - form event horizon ring
        this.equations = [
            { text: 'H = -Σp(x)log p(x)', angle: 0, dist: 0, delay: 0.3, formProgress: 0, atEdge: false },
            { text: 'Whole > Σ Parts', angle: Math.PI * 0.2, dist: 0, delay: 0.5, formProgress: 0, atEdge: false },
            { text: 'P(A|B) = P(B|A)P(A)/P(B)', angle: Math.PI * 0.4, dist: 0, delay: 0.7, formProgress: 0, atEdge: false },
            { text: 'rs = 2GM/c²', angle: Math.PI * 0.6, dist: 0, delay: 0.9, formProgress: 0, atEdge: false },
            { text: '∂S/∂t ≥ 0', angle: Math.PI * 0.8, dist: 0, delay: 1.1, formProgress: 0, atEdge: false },
            { text: 'θ = θ - α∇L(θ)', angle: Math.PI * 1.0, dist: 0, delay: 1.3, formProgress: 0, atEdge: false },
            { text: 'P(x) ∝ x^(-α)', angle: Math.PI * 1.2, dist: 0, delay: 1.5, formProgress: 0, atEdge: false },
            { text: 'I(X;Y) = H(X) - H(X|Y)', angle: Math.PI * 1.4, dist: 0, delay: 1.7, formProgress: 0, atEdge: false },
            { text: 'V ∝ n²', angle: Math.PI * 1.6, dist: 0, delay: 1.9, formProgress: 0, atEdge: false },
            { text: 'S = k log W', angle: Math.PI * 1.8, dist: 0, delay: 2.1, formProgress: 0, atEdge: false }
        ];
        this.equationTime = 0;
    ...
    drawEmergingPattern() {
        // Delay pattern appearance by 3 seconds
        this.patternDelay += 0.016; // ~60fps
        
        if (this.patternDelay < 3) return; // Don't draw for first 3 seconds
        
        // Fade in over 2 seconds after delay
        const fadeIn = Math.min((this.patternDelay - 3) / 2, 1);
        
        this.patternPhase += 0.015;
        
        // Pulse opacity between 0.08 and 0.16, multiplied by fadeIn
        this.patternOpacity = (0.08 + Math.sin(this.patternPhase) * 0.04) * fadeIn;
    ...
    drawFloatingEquations() {
        ...
        // Calculate edge distance based on screen size
        const edgeDist = Math.min(this.canvas.width, this.canvas.height) * 0.52;
        const invisibleZone = 100;
        
        this.equations.forEach(eq => {
            // Start after delay (FAST - 0.5 to 2.6 seconds)
            if (this.equationTime < eq.delay) return;
            
            // Move outward until reaching edge
            if (!eq.atEdge) {
                eq.dist += 4 * this.warpMultiplier; // Fast movement
                ...
                // Gentle orbit drift when at edge
                eq.angle += 0.001;
            }
            
            // Calculate position
            const screenX = this.centerX + Math.cos(eq.angle) * eq.dist * this.scrollSpread;
            const screenY = this.centerY + Math.sin(eq.angle) * eq.dist * this.scrollSpread;
            
            // Opacity: 0 at center, full at edge
            let opacity = 0;
            if (eq.dist > invisibleZone) {
                opacity = Math.min(0.6, (eq.dist - invisibleZone) / 150);
            }
            
            if (opacity < 0.02) return;
            
            // Form progress: binary chaos → equation
            eq.formProgress = Math.min(1, (eq.dist - invisibleZone) / 120);
            ...
            // Size - slightly larger when at edge
            const size = eq.atEdge ? 13 : 10 + (eq.dist / edgeDist) * 3;
            ...
            // Strong glow at edge (event horizon)
            if (eq.atEdge) {
                this.ctx.shadowBlur = 20;
                this.ctx.shadowColor = `rgba(255, 107, 53, ${opacity * 0.8})`;
            } else if (eq.formProgress > 0.5) {
                this.ctx.shadowBlur = 10;
                this.ctx.shadowColor = `rgba(255, 107, 53, ${opacity * 0.5})`;
            }
    ...
    drawPulseWave() {
        ...
        const pulseCycle = this.pulseTime % 8;
        
        if (pulseCycle < 2) {
            const progress = pulseCycle / 2;
            const radius = progress * Math.max(this.canvas.width, this.canvas.height);
    ...
    update() {
        ...
        // Check for idle mode (10 seconds)
        if (Date.now() - this.lastInteraction > 10000) {
            this.idleMode = true;
            console.log('IDLE MODE ACTIVE');
        }
        
        // Spiral movement when idle
        if (this.idleMode) {
            this.spiralAngle += 0.02;
            this.particles.forEach((p, i) => {
                const spiralOffset = (i / this.particles.length) * Math.PI * 2;
                p.x += Math.cos(this.spiralAngle + spiralOffset) * 2;
                p.y += Math.sin(this.spiralAngle + spiralOffset) * 2;
            });
        }
        
        this.particles.forEach(p => {
            p.z -= p.speed * this.warpMultiplier * this.scrollSpeed;
    ...
    draw() {
        console.log('BinaryTunnel: draw start');
        // Motion blur / trails
        ...
        this.ctx.shadowBlur = 0;
        console.log('BinaryTunnel: draw end');
    }
    ...
    animate() {
        console.log('BinaryTunnel: animate frame', this.particles?.length);
