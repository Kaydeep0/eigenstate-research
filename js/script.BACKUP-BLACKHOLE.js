...
class BinaryTunnel {
    ...
    drawFloatingEquations() {
        ...
        this.equations.forEach(eq => {
            ...
            if (!eq.atEdge) {
                eq.dist += 5 * this.warpMultiplier; // Fast movement
                
                // Check if reached edge (using normalized distance)
                if (eq.dist >= 1) {
                    eq.dist = 1;
                    eq.atEdge = true;
                }
            ...
            const screenX = this.centerX + Math.cos(eq.angle) * currentDistX * this.scrollSpread;
            const screenY = this.centerY + Math.sin(eq.angle) * currentDistY * this.scrollSpread;
            
            // Opacity: 0 at center, full at edge
            let opacity = 0;
            if (eq.dist > 0.2) {
                opacity = Math.min(0.7, (eq.dist - 0.2) / 0.5);
            }
            
            if (opacity < 0.02) return;
            
            // Form progress: binary chaos → equation
            eq.formProgress = Math.min(1, (eq.dist - 0.15) / 0.4);
            ...
            // LARGER SIZE
            const size = eq.atEdge ? 18 : 12 + eq.dist * 8;
